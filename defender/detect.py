...
# === New Rule: Impossible Travel ===
    for i in range(len(rows) - 1):
        ...
        if r1["user"] == r2["user"]:
            t1 = datetime.fromisoformat(r1["ts"].replace("Z", "+00:00"))
            t2 = datetime.fromisoformat(r2["ts"].replace("Z", "+00:00"))

            if r1["geo"] != r2["geo"] and abs((t2 - t1).total_seconds()) < 1800:
                findings.append((r2, f"impossible travel: {r1['geo']} → {r2['geo']}"))

    # === New Rule: Bulk Donor Table Read ===
    select_events = [r for r in rows if "SELECT" in r["note"]]

    for i in range(len(select_events) - 3):
        t1 = datetime.fromisoformat(select_events[i]["ts"].replace("Z", "+00:00"))
        t4 = datetime.fromisoformat(select_events[i+3]["ts"].replace("Z", "+00:00"))

        if (t4 - t1).total_seconds() < 10:
            findings.append((select_events[i+3], "bulk donor-table read (possible data harvesting)"))
            break
